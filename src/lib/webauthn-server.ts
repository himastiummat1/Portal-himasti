...
export async function getWebAuthnConfig() {
  ...
  // Ambil RP_ID dan Origin resmi, atau fallback ke host yang divalidasi
 ...
 * Dilengkapi tanda tangan HMAC terikat User ID untuk mencegah challenge hijacking
 */
export async function getRegistrationOptionsForUser(userId: bigint, email: string, name: string) {
  const { rpName, rpID } = await getWebAuthnConfig()

  // Ambil semua kredensial yang sudah terdaftar agar tidak duplicate
  ...
  const options = await generateRegistrationOptions({
    ...
    authenticatorSelection: {
      ...
      authenticatorAttachment: 'platform', // Wajib chip fisik perangkat (Fingerprint/FaceID/Knox)
    },
  })

  // Simpan challenge yang terikat HMAC ke user_id
...
export async function verifyAndSaveRegistration(
  ...
  // Hapus cookie segera (Anti-Replay)
  cookieStore.delete(CHALLENGE_COOKIE_NAME)

  if (!rawCookie) {
    throw new Error('Sesi pendaftaran biometrik telah kedaluwarsa atau tidak valid. Silakan ulangi.')
  }

  // Verifikasi HMAC dan bind ke user aktif
  ...
  // Simpan ke tabel webauthn_credentials
  const savedCred = await prisma.webauthn_credentials.create({
    data: {
      user_id: userId,
...
/**
 * Generate opsi autentikasi untuk presensi acara
 */
export async function getAuthenticationOptionsForAttendance(userId?: bigint) {
  ...
  const options = await generateAuthenticationOptions({
    rpID,
    userVerification: 'required', // Wajib otorisasi biometrik fisik
    allowCredentials,
  })

  // Simpan challenge terikat HMAC ke user_id
...
export async function verifyAttendanceAssertion(
  ...
  // Cari kredensial publik hardware yang cocok
  const credentialRecord = await prisma.webauthn_credentials.findUnique({
    where: { credential_id: response.id },
  })

  // VALIDASI KETAT ANTI-JOKI: Chip fisik harus benar-benar terdaftar milik user aktif
  if (!credentialRecord || credentialRecord.user_id.toString() !== userId.toString()) {
  ...
  // Update counter dan last_used_at untuk mencegah serangan replay kloning passkey
