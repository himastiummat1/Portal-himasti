...
if (!content.includes('Trophy')) {
  content = content.replace('LayoutGrid', 'LayoutGrid, Trophy');
}
if (!content.includes('TerminalEasterEgg')) {
  content = content.replace('import { ExternalLink }', 'import TerminalEasterEgg from "./TerminalEasterEgg";\nimport { ExternalLink }');
}

// 2. Add Leaderboard Widget Data & Component inside the right column for both KaderView and AdminView.
// First, create the Leaderboard Widget string.
...
// Inject into KaderView right column (before Radar Hackathon)
content = content.replace(
  '{/* Radar Hackathon - Widget Kolom Kanan */}\n        <div className="col-span-1 lg:col-span-1 space-y-6">',
  `{/* Radar Hackathon - Widget Kolom Kanan */}\n        <div className="col-span-1 lg:col-span-1 space-y-6">\n${leaderboardWidget}`
);

// Inject into AdminView right column (before Radar Hackathon)
content = content.replace(
  '{/* Kolom Kanan (Agenda & Radar) */}\n        <div className="col-span-1 lg:col-span-4 space-y-6">',
  `{/* Kolom Kanan (Agenda & Radar) */}\n        <div className="col-span-1 lg:col-span-4 space-y-6">\n${leaderboardWidget}`
);

// 3. Inject TerminalEasterEgg right before the final closing div of the page
const terminalInjection = `      <TerminalEasterEgg userName={session.user.name || "Kader"} />\n    </div>`;
