...
export default function DigitalKTA({ 
  ...
  const CardContent = ({ zoomed = false }) => (
    <>
      ...
      <div className={`flex gap-4 items-end justify-between w-full relative z-10 ${zoomed ? 'mt-12 md:mt-auto md:mb-auto' : 'mt-6'}`} style={{ transform: "translateZ(50px)" }}>
        <div className="flex-1 min-w-0 pr-4">
          <div className="mb-4">
            ...
            <div className={`${zoomed ? 'text-5xl md:text-6xl lg:text-7xl whitespace-normal break-words leading-tight' : 'text-xl truncate leading-tight'} font-bold text-white w-full`}>{name}</div>
          </div>
          
          <div className={`flex flex-wrap ${zoomed ? 'gap-16 mt-8' : 'gap-4 mt-2'}`}>
            <div>
              <div className={`${zoomed ? 'text-sm mb-2' : 'text-[9px] mb-0.5'} text-slate-400 font-mono uppercase tracking-widest`}>NIM</div>
              <div className={`${zoomed ? 'text-2xl md:text-3xl' : 'text-sm'} font-mono font-bold text-slate-200`}>{nim}</div>
            </div>
            <div>
              <div className={`${zoomed ? 'text-sm mb-2' : 'text-[9px] mb-0.5'} text-slate-400 font-mono uppercase tracking-widest`}>ANGKATAN</div>
              <div className={`${zoomed ? 'text-2xl md:text-3xl' : 'text-sm'} font-mono font-bold text-slate-200`}>{angkatan}</div>
            </div>
          </div>
        </div>

        <div className={`p-2 border border-white/10 bg-white shrink-0 shadow-2xl flex items-center justify-center ${zoomed ? 'p-5 rounded-2xl w-fit' : 'rounded-xl'}`} style={{ transform: "translateZ(60px)" }}>
          <QRCodeSVG value={qrValue} size={zoomed ? 200 : 64} level="H" />
        </div>
      </div>

      <div className={`w-full flex justify-between items-end border-t border-white/10 relative z-10 ${zoomed ? 'mt-8 pt-6 gap-6' : 'mt-4 pt-3 gap-2'}`} style={{ transform: "translateZ(20px)" }}>
        ...
        <AnimatePresence>
          {isZoomed && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-12 overflow-hidden">
              ...
              {/* Massive Centered Card (Keeps the card aspect ratio!) */}
              <div 
                className="relative z-10 w-full max-w-6xl perspective-1000 flex items-center justify-center"
                style={{ perspective: "2000px" }}
              >
                <button 
                  onClick={() => setIsZoomed(false)}
                  className="absolute -top-16 right-0 w-12 h-12 bg-white/10 border border-white/20 text-white rounded-full flex items-center justify-center hover:bg-white hover:text-black shadow-2xl transition-all duration-300 z-[9999] cursor-pointer"
                >
                  <X className="w-6 h-6" />
                </button>
                ...
                <motion.div
                  ...
                  className="w-full relative aspect-[1.6/1] md:aspect-[2/1] lg:aspect-[2.2/1] bg-black rounded-[2rem] p-8 md:p-14 shadow-[0_30px_100px_rgba(0,0,0,0.5)] flex flex-col justify-between border border-slate-800"
                >
