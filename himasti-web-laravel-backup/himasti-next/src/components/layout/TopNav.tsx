      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex items-center" ref={navRef}>
            <Link href="/admin" className="font-extrabold text-2xl text-purple-600  mr-8 tracking-tight">
              HIMASTI
            </Link>
            
            {/* Desktop Menu */}
            <div className="hidden md:flex space-x-1">
              <Link href="/admin" className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 flex items-center gap-1 ${pathname === "/admin" ? "bg-purple-50 text-purple-700  " : "text-gray-600  hover:bg-gray-100 hover:text-purple-600 :bg-gray-700"}`}>Portal Informasi</Link>
                    ...
                    <button 
                      ...
                      className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 flex items-center gap-1 ${
                        isActive || isOpen
                          ...
                          : "text-gray-600  hover:bg-gray-100 hover:text-purple-600 :bg-gray-700"
                      }`}
                    >
                      {group.title}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-gray-200  bg-white ">
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 overflow-y-auto max-h-[70vh]">
            <div className="mb-4"><Link href="/admin" onClick={() => setMobileMenuOpen(false)} className={`block px-3 py-2 rounded-md text-base font-medium ${pathname === "/admin" ? "bg-purple-50 text-purple-700 " : "text-gray-700  hover:bg-gray-50"}`}>Portal Informasi</Link></div>
            {groups.map((group) => (
